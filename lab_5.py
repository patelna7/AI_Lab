...
def print_schedule(schedule, num_days, max_lessons_per_day, num_classes, class_names):
    ...
    table.field_names = [line_color + "День", "Номер уроку"] + class_names[:num_classes]
    dni = ["Понеділок","Вівторок","Середа","Четвер","П'ятниця"]
    for day_index in range(num_days):
        for lesson_index in range(max_lessons_per_day):
            ...
            table.add_row(
                [f"{dni[day_index]}" if lesson_index == 0 else " ", f"Урок  {lesson_index + 1}"] +
...
def main():
    ...
    teachers = [
        Teacher("Білик М.М.", ["Математика", "Німецька", "Фізика", "Історія"]),
        Teacher("Трущак О.О.", ["Історія", "Німецька", "Трудове", "Малювання"]),
        Teacher("Хомякова Р.В.", ["Математика", "Німецька", "Фізика", "Історія"]),
        Teacher("Сандаль Д.П.", ["Фізкультура", "Трудове"]),
    ]

    room_names = ["114", "232", "554", "Спортзал", "Майстерня", "Клас малювання"]

    class_names = ["7-А", "7-Б", "7-В"]

    lesson_names = [["Математика", False, "None"], ["Німецька", False, "None"], ["Фізика", False, "None"],
                    ["Історія", False, "None"], ["Фізкультура", True, "Спортзал"], ["Малювання", True, "Клас малювання"],
                    ["Трудове", True, "Майстерня"]]
